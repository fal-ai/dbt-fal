...
def fal_flow_run(parsed):
    ...
    for node_graph in sub_graphs:
        ...
        run_scripts(_id_to_fal_scripts(node_graph, local_befores), project)

        if len(bigger_plan.dbt_models) != 0:
            dbt_run(parsed, _unique_ids_to_model_names(local_dbts))

        run_scripts(_id_to_fal_scripts(node_graph, local_afters), project)
