...
def _isolated_runner(
    ...
    local_packages: bytes | None = None,
) -> Any:
    ...
    if local_packages is not None:
        ...
        assert (
            not fal_scripts_path.exists()
        ), f"Path: {fal_scripts_path} already exists in isolate cloud."
...
def run_in_environment_with_adapter(
    ...
    if type(environment).__name__ in ["IsolateServer", "FalHostedServer"]:
        if (
            type(config.credentials).__name__ == "BigQueryCredentials"
            and str(config.credentials.method) == "service-account"
        ):
            raise RuntimeError(
        ...
        deps = [
            ...
            for i in get_default_pip_dependencies(
                is_remote=True, adapter_type=adapter_type
            )
        ]
