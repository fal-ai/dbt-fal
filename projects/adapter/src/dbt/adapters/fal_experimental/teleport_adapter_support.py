...
def wrap_db_adapter(
    db_adapter: BaseAdapter, teleport_credentials: TeleportCredentials
) -> TeleportAdapter:
    ...
    if db_adapter.type() == "snowflake":
        ...
        if teleport_credentials.type == TeleportTypeEnum.REMOTE_S3:
            return support_snowflake.SnowflakeAdapterTeleport(
                db_adapter, teleport_credentials
            )
        else:
            raise RuntimeError("Snowflake teleporting works only with S3.")

    raise NotImplementedError(
        f"Teleport support has not been implemented for adapter {db_adapter.type()}"
    )
